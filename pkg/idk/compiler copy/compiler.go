...
func (c *Compiler) CompileProgram() []byte {
	program := make([]byte, 0)
	...
	for !c.next.Is(token.EOF) {
		...
		program = append(program, c.compileStatement()...)
		c.skipEols()
	}
...
func (c *Compiler) compileStatement() []byte {
	...
	case c.current.Is(token.IDENTIFIER) && c.next.Is(token.DECLASSIGN):
		return c.compileDeclareAssignStatement()
	...
	case c.current.Is(token.IDENTIFIER) && c.next.Is(token.LPARENTHESIS):
		return c.compileFunctionCallStatement()
...
func (c *Compiler) compileDeclareAssignStatement() []byte {
	...
	bytecode = append(bytecode, opcodes.VarBind(varType))
	bytecode = append(bytecode, byte(len(identifier.Value)))
	bytecode = append(bytecode, []byte(identifier.Value)...)
...
func (c *Compiler) compileFunctionCallStatement() []byte {
	bytecode, _ := c.compileFunctionCallExpression()
	return bytecode
}
...
func (c *Compiler) compileExpression(precedence int) ([]byte, opcodes.Type) {
...
func (c *Compiler) compilePrefixExpression() ([]byte, opcodes.Type) {
	bytecode := make([]byte, 0)
...
func (c *Compiler) compileInfixExpression(left []byte) ([]byte, opcodes.Type) {
	bytecode := make([]byte, 0)
...
func (c *Compiler) compileIdentifier() ([]byte, opcodes.Type) {
...
func (c *Compiler) compileFunctionCallExpression() ([]byte, opcodes.Type) {
	bytecode := make([]byte, 0)
	...
	case "print":
		c.advance()
		lit, varType := c.compileExpression(LOWEST)
		bytecode = append(bytecode, lit...)
...
func (c *Compiler) compileIdentifierLiteral() ([]byte, opcodes.Type) {
	bytecode := make([]byte, 0)

	varName := c.current.Value // get variable type
	varType, ok := c.currentScope.Lookup(varName)

	if !ok {
		// handle error
	}

	bytecode = append(bytecode, opcodes.VarLookup(varType))
...
func (c *Compiler) compileIntegerLiteral() ([]byte, opcodes.Type) {
	bytecode := make([]byte, 0)
...
func (c *Compiler) compileFloatingPointLiteral() ([]byte, opcodes.Type) {
	bytecode := make([]byte, 0)
...
func (c *Compiler) compileBooleanLiteral() ([]byte, opcodes.Type) {
	bytecode := make([]byte, 0)
...
func (c *Compiler) compileCharacterLiteral() ([]byte, opcodes.Type) {
	bytecode := make([]byte, 0)
...
func (c *Compiler) compileStringLiteral() ([]byte, opcodes.Type) {
	bytecode := make([]byte, 0)

	bytecode = append(bytecode, opcodes.SPUSH)
	bytecode = append(bytecode, byte(len(c.current.Value)))
	bytecode = append(bytecode, []byte(c.current.Value)...)
...
func (c *Compiler) reportTypeMismatch(operator token.Token, type1, type2 opcodes.Type) {
